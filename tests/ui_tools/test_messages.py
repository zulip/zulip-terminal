...
class TestMessageBox:
    ...
    def test_init(
        ...
        msg_box = MessageBox(cast(Message, message), self.model, None)
    ...
    def test_init_fails_with_bad_message_type(self) -> None:
        ...
        with pytest.raises(RuntimeError):
            MessageBox(cast(Message, message), self.model, None)
    ...
    def test_private_message_to_self(self, mocker: MockerFixture) -> None:
        ...
        cast(MagicMock, msg_box._is_private_message_to_self).assert_called_once_with()
    ...
    def test_soup2markup(
        self, content: str, expected_markup: List[Any], mocker: MockerFixture
    ) -> None:
        mocker.patch(
    ...
    def test_main_view(
        self, mocker: MockerFixture, message: Message, last_message: Optional[Message]
    ) -> None:
    ...
    def test_main_view_renders_slash_me(
        self, mocker: MockerFixture, message: Message, content: str, is_me_message: bool
    ) -> None:
    ...
    def test_main_view_content_header_without_header(
        ...
        msg_box = MessageBox(cast(Message, this_msg), self.model, last_msg)
    ...
    def test_main_view_compact_output(
        ...
        msg_box = MessageBox(cast(Message, varied_message), self.model, varied_message)
        view_components = msg_box.main_view()
        assert len(view_components) == 1
        assert isinstance(view_components[0], Padding)

    def test_main_view_generates_EDITED_label(
        self, mocker: MockerFixture, messages_successful_response: Dict[str, Any]
    ) -> None:
        messages = messages_successful_response["messages"]
    ...
    def test_keypress_EDIT_MESSAGE(
        ...
        msg_box = MessageBox(cast(Message, varied_message), self.model, message_fixture)
        size = widget_size(msg_box)
    ...
    def test_transform_content(
        self, mocker: MockerFixture, raw_html: str, expected_content: str
    ) -> None:
    ...
    def test_reactions_view(
        ...
        varied_message = dict(message_fixture, **to_vary_in_each_message)
        msg_box = MessageBox(cast(Message, varied_message), self.model, None)
    ...
    def test_footlinks_limit(
        self, maximum_footlinks: int, expected_instance: Type[Any]
    ) -> None:
        message_links = OrderedDict(
            [
