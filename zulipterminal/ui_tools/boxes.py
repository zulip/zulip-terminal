...
from zulipterminal.config.keys import (
    ...
    primary_keyboard_key_for_command,
)
    ...
    def _tidy_valid_recipients_and_notify_invalid_ones(
        ...
        if invalid_recipients:
            invalid_recipients_error = [
                ...
                " - Use ",
                ("footer_contrast", primary_keyboard_key_for_command("AUTOCOMPLETE")),
                " or ",
                (
                    "footer_contrast",
                    primary_keyboard_key_for_command("AUTOCOMPLETE_REVERSE"),
                ),
                " to autocomplete.",
            ]
                        ...
                        if not self.model.is_valid_stream(stream_name):
                            invalid_stream_error = (
                                ...
                                " Use {} or {} to autocomplete.".format(
                                    primary_keyboard_key_for_command("AUTOCOMPLETE"),
                                    primary_keyboard_key_for_command(
...
class MessageSearchBox(urwid.Pile):
    ...
    def main_view(self) -> Any:
        search_text = (
            f"Search [{', '.join(keyboard_keys_for_command('SEARCH_MESSAGES'))}]: "
        )
...
class PanelSearchBox(urwid.Edit):
    ...
    def __init__(
        ...
        self.search_text = (
            f" Search [{', '.join(keyboard_keys_for_command(search_command))}]: "
        )
