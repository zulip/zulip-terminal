...
# Adapted from https://github.com/rndusr/stig/blob/master/stig/tui/scroll.py
...
class Scrollable(urwid.WidgetDecoration):
    ...
    def _adjust_trim_top(self, canv, size):
        ...
        # still scroll out
        if self._old_cursor_coords is not None and \
                self._old_cursor_coords != canv.cursor:
            self._old_cursor_coords = None
            curscol, cursrow = canv.cursor
    ...
    def rows_max(self, size=None, focus=False):
        ...
        If `size` is not given, the currently rendered number
        of rows is returned.
        """
        if size is not None:
            ow = self._original_widget
...
class ScrollBar(urwid.WidgetDecoration):
    ...
    def __init__(self, widget, thumb_char=u'\u2588', trough_char=' ',
        ...
        `widget` must be a box widget with the following methods:
          ...
          - `set_scrollpos` (optional; needed for mouse click support) takes
            the index of the first visible row.
          - `rows_max` takes `size` and `focus` and returns the total number
            of rows `widget` can render.
    ...
    def render(self, size, focus=False):
        ...
        # "cviews do not fill gaps in shard_tail!" or "cviews overflow
        # gaps in shard_tail!" exceptions. Stacking the same SolidCanvas
        # is a workaround.
