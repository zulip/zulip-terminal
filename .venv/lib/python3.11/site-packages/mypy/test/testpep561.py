...
def virtualenv(python_executable: str = sys.executable) -> Iterator[tuple[str, str]]:
    ...
    with tempfile.TemporaryDirectory() as venv_dir:
        proc = subprocess.run(
            [python_executable, "-m", "venv", venv_dir], check=False, cwd=os.getcwd(), capture_output=True
        )
...
def upgrade_pip(python_executable: str) -> None:
    ...
    try:
        with filelock.FileLock(pip_lock, timeout=pip_timeout):
            proc = subprocess.run(install_cmd, check=False, capture_output=True, env=os.environ)
...
def install_package(
    ...
    with tempfile.TemporaryDirectory() as dir:
        ...
        try:
            with filelock.FileLock(pip_lock, timeout=pip_timeout):
                proc = subprocess.run(install_cmd, check=False, cwd=working_dir, capture_output=True, env=env)
        except filelock.Timeout as err:
            raise Exception(f"Failed to acquire {pip_lock}") from err
    if proc.returncode != 0:
        raise Exception(proc.stdout.decode("utf-8") + proc.stderr.decode("utf-8"))
