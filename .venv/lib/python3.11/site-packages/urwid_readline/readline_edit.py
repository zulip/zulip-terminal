...
class ReadlineEdit(urwid.Edit):
    ...
    def backward_kill_line(self):
        for pos in reversed(range(self.edit_pos)):
    ...
    def end_of_line(self):
        ...
        for pos in range(self.edit_pos, text_length + 1):
            if pos == text_length or self.edit_text[pos] == "\n":
                self.set_edit_pos(pos)
                return
    ...
    def _complete(self, cycle_forward):
        if self._autocomplete_state:
            if self._autocomplete_state.num == 0 and not cycle_forward or self._autocomplete_state.num == -1 and cycle_forward:
